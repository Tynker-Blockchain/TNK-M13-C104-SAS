...
class Wallet():
    ...
    def deleteAccountName(self, account_address):
        # Initialize a reference to your Firebase database
        ref = db.reference('/accounts')  # Change '/accounts' to your Firebase database path
        # Get the account node by its address
        account_node = ref.child(account_address)
        # Update the account node to remove the account name
        account_node.update({"accountName": None})
